...
class Map:
    ...
    def display(self, screen:pygame.surface.Surface) -> None:
        ...
        for b in self.dict_name_build[-1].get("ExitGallery", []):
            b.transport(self)
    ...
    def update_galleries_links(self) -> None:        
        for b in self.dict_name_build[-1].get("EntranceGallery", []):
            l = b.update_links(self, [])
            del l
